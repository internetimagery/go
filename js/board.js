(function() {
  ...
  Resize = function(x, y, w, h, style) {
    return this.setAttribute("style", ("margin-left:" + x + "%;margin-top:" + y + "%;width:" + w + "%;height:" + h + "%;") + style);
  };
  ...
  Board = (function() {
    function Board(element, size) {
      var center, col, grid_chunk, inner, inner_frame_pos, inner_frame_span, row, socket, star, star_size, stars, stone_size, _fn, _i, _j, _k, _l, _len, _m, _ref, _ref1, _ref2, _ref3, _ref4;
      this.size = size;
      this.callbacks = [];
      this.stone_class = ["empty", "stone set black", "stone set white", "empty-black", "empty-white"];
      this.board_state = [];
      if (this.size < 2) {
      ...
      if ((7 < (_ref2 = this.size) && _ref2 < 13)) {
        stars.push([2, 2]);
        stars.push([2, this.size - 3]);
        stars.push([this.size - 3, 2]);
        stars.push([this.size - 3, this.size - 3]);
      }
      if (13 <= this.size) {
        stars.push([3, 3]);
        stars.push([3, this.size - 4]);
        stars.push([this.size - 4, 3]);
        stars.push([this.size - 4, this.size - 4]);
      }
      for (col = _k = 0, _ref3 = this.size; 0 <= _ref3 ? _k < _ref3 : _k > _ref3; col = 0 <= _ref3 ? ++_k : --_k) {
        _fn = (function(_this) {
          return function() {
            var pos;
        ...
        for (row = _l = 0, _ref4 = this.size; 0 <= _ref4 ? _l < _ref4 : _l > _ref4; row = 0 <= _ref4 ? ++_l : --_l) {
          for (_m = 0, _len = stars.length; _m < _len; _m++) {
            star = stars[_m];
